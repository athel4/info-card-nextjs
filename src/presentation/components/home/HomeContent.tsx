...
import { Brain, Shield, Sparkles, Users, Clock, RefreshCw } from 'lucide-react';
...
const HomeContent = () => {
  ...
  }, [dailyCredits, creditInfo, getTotalRemainingCredits]);
  ...
  const processBusinessCards = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Successfully processed ${extractedData.length} business card${extractedData.length > 1 ? 's' : ''} using ${creditsUsed || 0} credit${(creditsUsed || 0) > 1 ? 's' : ''}.`,
      });
  ...
  }, [totalCredits, dailyCredits, creditInfo, isAnonymous, hasShownModal, startupCredit]);
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-6">
            <div className="p-4 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full shadow-lg relative">
              <img src="/og-image.png" className='invisible' style={{ maxWidth: "60px", maxHeight: "50px" }} alt="Logo Placeholder" / >
              <img src="/og-image.png" className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' style={{  maxHeight: "80px" }} alt="Logo" / >
            </div>
          </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-center gap-3 p-4 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-sm text-green-800 text-center md:text-left">
                  <a href="/tnc_with_privacy.pdf" target='_blank' rel="noopener noreferrer" className="hover:underline cursor-pointer">Term &amp; Condition</a>
                </p>
              </div>
            </div>
          </div>
